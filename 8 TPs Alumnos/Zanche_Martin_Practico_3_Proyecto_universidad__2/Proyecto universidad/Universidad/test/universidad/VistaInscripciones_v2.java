 ...
 * @author Martin
 */
public class VistaInscripciones_v2 extends javax.swing.JInternalFrame {
 ...
 //private ArrayList<int> materiasDelAlumno;
   ...
   ArrayList<Object> columns= new ArrayList<>();
            ...
            if (e.getType()== TableModelEvent.UPDATE){// &&  e.getColumn()==5){
                ...
                if (columna == 5){
                    if (!(boolean)tblMaterias.getValueAt(fila, 5)){
                        ...
                        int id= (int)tblMaterias.getValueAt(fila, 6);
                       // cd.buscarCursadaXId(id);
    ...
    private void btnIncribirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIncribirActionPerformed
       ...
       for (int j=0;j<largo;j++){
            try {
               ...
               Logger.getLogger(VistaInscripciones_v2.class.getName()).log(Level.SEVERE, null, ex);
           }
       }
    ...
    private void btnAnularActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAnularActionPerformed
       ...
       int largo=idCursadasAEliminar.size();
        if(largo>0){
       for (int j=0;j<largo;j++){
            try {
                cd.borrarCursadaPorIdDeCursada(idCursadasAEliminar.get(j));
               
           } catch (SQLException ex) {
               Logger.getLogger(VistaInscripciones_v2.class.getName()).log(Level.SEVERE, null, ex);
           }
       }
        JOptionPane.showMessageDialog(null, "Se elimino inscripcion a materias seleccionadas", "Info", JOptionPane.WARNING_MESSAGE); 
       // borrarFilasTabla();
        modelo.fireTableDataChanged();//refresca la vista de la tabla
       // i=0;
       idCursadasAEliminar.clear();
       listaCursada.clear();
       listaCursada=(ArrayList)cd.obtenerCursadas();
       borrarFilasTabla();
       cargarDatos();
       } else JOptionPane.showMessageDialog(null, "No elimino inscripcion a ninguna materia", "Info", JOptionPane.WARNING_MESSAGE); 
